...
class upCAE
{
    public function updateCAE()
    {
        ...
        for ($n = 0; $n < ceil($num / 320); $n++) {
            ...
            foreach ($urls as $i => $url) {
                ...
                //querylist院士简历页面
                $info = $this->QLCAE($data);
                //更新一个院士$string:职业，$array:院士信息
                $array_job = array('中国工程院院士');
    ...
    public function url()
    {
        require_once Root_Path . '/Backend/Script/learning/CAE.php';
        $obj = new cae();
        return $obj->get();
    }
    //querylist院士简历页面
    public function QLCAE($data)
    {
        $rules = array(
            'name'  => array('.tion_title', 'text'),
            'photo' => array('.detail_info>.info_img>img', 'src'),
            'intr'  => array('.detail_info>.intro', 'text'),
        );
        @$data = QueryList::Query($data, $rules)->getData(function ($item) {
            $item['name']  = trim(mb_convert_encoding($item['name'], "UTF-8", "GB2312"));
            $item['photo'] = 'http://www.cae.cn' . mb_convert_encoding($item['photo'], "UTF-8", "GB2312");
            $item['intr']  = trim(mb_convert_encoding($item['intr'], "UTF-8", "GB2312"));
            return $item;
        });
        return $data;
    }
